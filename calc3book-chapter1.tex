...
\begin{figure}[h]
 \begin{center}
  \includegraphics{fig1.2.1.0}\vspace{-3mm}
 \end{center}
 \caption[]{}
 \label{fig:scalar}
...
\begin{figure}[h]
 \begin{center}
  \includegraphics{fig1.4.2.0}\vspace{-5mm}
 \end{center}
...
\begin{exmp}\label{exmp:crossarea}
 ...
 \begin{figure}[h]
 \begin{center}
  \includegraphics{fig1.4.3.0}\vspace{-5mm}
 \end{center}
 \caption[]{}
 \label{fig:crossarea}
\end{figure}
...
We will first consider lines\index{line}.\vspace{4mm}
...
coordinates are useful when there is symmetry about the origin.U

If a problem is given in curvilinear coordinates, 
the typical solution consist of (1) converting the data in Cartesian coordinates, (2) solving it in the Cartesian coordinates and (3) converting the results back to the original curvilinear coordinates.
...
$xy$-plane. 
Treating $(x,y)$ as a point in $\Real{2}$, let $(r,\theta)$ be its polar coordinates (see
Figure 1.7.2). 
Let\index{coordinates!polar}
$\rho$ be the length of the line segment from the origin to $P$, and let $\phi$ be the angle between that line segment
and the positive $z$-axis (see Figure 1.7.3). $\phi$ is called the \emph{zenith angle}\index{zenith angle}. 
Then the
...
\begin{exmp}
 ...
 This is a parametric equation of a \emph{helix}\index{helix} (see Figure 1.8.1). As the value of $t$ increases, the terminal points of
...
%Begin Section 1.9
\section{Arc Length}
Let $\textbf{r}(t) = (x(t),y(t),z(t))$ be the position vector of an object moving in $\Real{3}$. 
...
\end{equation}
This is analogous to the case from single-variable calculus for parametric functions in $\Real{2}$. This is indeed how
...
(or a $C^{\infty}$ function). All the functions we will consider will be
...
If there are values of $t$ in the interval $[a,b]$ where the derivative of a
...
In this case, the arc length over $[a,b]$ is defined as
the sum of the arc lengths over the subintervals.

Notice that the set traced out by the function $\textbf{f}(t) = (\cos t , \sin t , t)$ from Example
...
{Let $\textbf{f}(t)$ be a smooth curve in $\Real{3}$ defined on an interval
 ...
 If $\alpha$ positive derivatives then we
...
into the formula for $\textbf{f}(t)$ to get $\textbf{g}(s)$.
...
The arc length parametrization plays an important role in the field of mathematics known as \emph{differential
geometry}.\footnote{See \cite{one} for an introduction to elementary differential geometry.}
If the smooth curve $\textbf{g}$ parametrized by arc length, 
then the value
\[\kappa=\norm{\textbf{g}\,''(s)}\] 
are called \index{curvature}\emph{curvature} of curve $\textbf{g}$ at the point $\textbf{g}(s)$.
The curvature $\kappa$ describes geometric shape of the set traced by $\textbf{g}$;
for example if $\textbf{g}$ parametrize a circle of radius $R$ then $\kappa=\frac1R$ and the straight lines have curvature zero.

Assume you want to calculate curvature of the given curve using the definition above.
...
calculate at all.}
The following theorem provides a simple way to calculate the curvature directly, without passing to the reparametrization
...
\begin{proofbar}\vspace{-3mm}\begin{proof}[Proof:]
Let $\textbf{g}(s)$ be the arc length parametrization of $\textbf{f}(t)$.
...
\end{align*}
Since $\Crossprod{\textbf{f}\,'(t)}{\textbf{f}\,'(t)}=0$,
we get
\begin{align*}
...
That is $\textbf{g}\,''(s)\perp\textbf{g}\,'(s)$ for any $s$ and therefore
\begin{align*}
\frac{\norm{\Crossprod{\textbf{f}\,''(t)}{\textbf{f}\,'(t)}}}{\norm{\textbf{f}\,'(t)}^3}
&=
