...
If \textbf{f} is continuous (see the definition below) then it is also called a \emph{curve};
in this case we identify vector $\textbf{f}(t)$ with its terminal points.
...
\begin{exmp}
 ...
 $\textbf{f}(t)$ trace out a curve spiraling upward. 
...
\end{equation}
This this integral will be used to define the arc length of a curve in $\Real{3}$.
...
smooth. 
A \emph{smooth curve} $\textbf{f}(t)$ is one whose component functions are all smooth;
if in addition the derivative $\textbf{f}\,'(t)$ is never the zero vector then it is called \emph{smooth regular curve}.

We will be mostly interested in the curves without self-intersections;
that is $\textbf{f}(t_1)\ne \textbf{f}(t_2)$ if $t_1\ne t_2$.
...
Let consider the set traced out by the given curve $\textbf{f}(t)$.
...
{Let $\textbf{f}(t)$ be a smooth curve in $\Real{3}$ defined on an interval
 ...
 If $\alpha$ positive derivatives 
 then we say that the reparametrization is \emph{regular reparametrization}\emph{regular} and 
...
This is a circle lying in the osculating plane,
its center is lies in the direction of curvature vector $\textbf{g}''(s)$ from $\textbf{g}(s)$ on the distance $R(s)$;
if $\kappa(s)=0$ then it degenerates to the tangent line.
...
\resume{enumerate}[{[\bfseries 1.]}]
  ...
  \item Use this formula to get an other proof of Theorem~\ref{thm:curvature-expressions}.
   \end{enumerate}
   \item Let $\textbf{g}(s)$ be a smooth curve with arc length parametrization and $\kappa(s)$ be its curvature.
  Show that 
  \[\Dotprod{\textbf{g}\,'''(s)}{\textbf{g}\,'(s)}=-\kappa(s)^2.\]
  
  \item Let $\textbf{g}$ be a smooth curve with arc length parametrization defined on $[0,a]$.
  The curve $\textbf{h}(s)=\textbf{g}(s)-s\textbf{g}'(s)$ is called \index{involute}\emph{involute} of $\textbf{g}(s)$.
  \begin{enumerate}[(a)]
  \item Show that 
  \[\norm{\textbf{h}'(s)}=s\,\kappa(s)\] 
  for $0<s<a$, where $\kappa(s)$ is curvature of $\textbf{g}$ at $\textbf{g}(s)$
  \item Show that the curvature of $\textbf{h}$ at $\textbf{h}(s)$ equals to $\tfrac1s$ for $0<s<a$.
  \end{enumerate}
\suspend{enumerate}
\probs{C}
\resume{enumerate}[{[\bfseries 1.]}]
