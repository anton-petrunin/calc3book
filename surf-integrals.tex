...
\statecomment{
 {The following statements are equivalent for a simply connected solid region $S$ in $\Real{3}$:
  ...
  \item $\dfrac{\partial R}{\partial y} = \dfrac{\partial Q}{\partial z}$,
   ...
   $\dfrac{\partial Q}{\partial x} = \dfrac{\partial P}{\partial y}$ in $S$ (i.e. $\curl\mathbf{f} =
   \mathbf{0}$ in $S$)
...
(i.e. $\Sigma$ is orientable and its boundary is $C$). Since $\nabla f$ is a vector field, then
\begin{align*}
 ...
 \item Use $\mathbf{f} = u\,\nabla v$ in the Divergence Theorem to prove:
  \begin{enumerate}[(a)]
   ...
   \item \emph{Green's second identity:}
    ...
    \iint_{\Sigma} \Dotprod{(u\,\nabla v ~-~ v\,\nabla u)}{d\bm{\sigma}}\]
  \end{enumerate}
 \item Suppose that $\Delta\,u = 0$ (i.e. $u$ is \emph{harmonic}) over $\Real{3}$.\index{harmonic}  
  Show that 
