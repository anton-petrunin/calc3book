...
In Section 1.8 we discussed vector-valued functions of a single real variable. We will now examine real-valued
functions of a point (or vector) in $\Real{2}$ or $\Real{3}$. For the most part these functions will be defined
on sets of points in $\Real{2}$, but there will be times when we will use points in $\Real{3}$, and there will also be
...
$(x,y)$ in $D$ a real number $f(x,y)$. The largest possible set $D$ in $\Real{2}$ on which $f$ is defined is
...
\begin{exmp}
 ...
 $\Real{2}$ ($D$ is sometimes called the \emph{closed unit disk}\index{unit disk}). The range of $f$ is the interval
...
$\mathbb{R}$, are called the \textbf{level curves}\index{level curve} of the function. Equivalently, the level
...
\statedefn{defn:mlim}{\index{limit}
 ...
 containing $(a,b)$ (but not necessarily defined at $(a,b)$ itself). Then we say that the \textbf{limit} of
...
\statethm{thm:mlimalg}{
 ...
 exist, and that $k$ is some scalar. Then:
 \begin{enumerate}[(a)]
...
\resume{enumerate}[{[\bfseries 1.]}]
 \item Show that $f(x,y)= \frac{1}{2\pi\sigma^2}e^{-(x^2 + y^2 )/2\sigma^2}$, for $\sigma >0$, is constant on the circle
  of radius $r > 0$ centered at the origin. This function is called a \emph{Gaussian blur}\index{Gaussian blur}, and is
  used as a filter in image processing software to produce a ``blurred'' effect.
 \item Suppose that $f(x,y) \le f(y,x)$ for all $(x,y)$ in $\Real{2}$. Show that $f(x,y) = f(y,x)$ for all
  $(x,y)$ in $\Real{2}$.
...
\probs{C}
\resume{enumerate}[{[\bfseries 1.]}]
 \item Prove Theorem \ref{thm:mlimalg}(a) in the case of addition. (\emph{Hint: Use Definition \ref{defn:mlim}.})
...
start to develop an idea of a derivative of a function of two or more variables. We will start with the notion of a
\emph{partial derivative}.\index{partial derivative}\index{derivative!partial}\index{$\dfrac{\partial f}{\partial x}$}

\statedefn{defn:partial}{\index{$\partial$}
 {Let $f(x,y)$ be a real-valued function with domain $D$ in $\Real{2}$, and let $(a,b)$ be a point in $D$. Then the
  \textbf{partial derivative of $\bm{f}$ at $\bm{(a,b)}$ with respect to $\bm{x}$}, denoted by
...
}\footnotetext{It is not a Greek letter. The symbol was first used by the mathematicians A. Clairaut and L. Euler
...
\statedefn{defn:dirderiv}{\index{directional derivative}\index{derivative!directional}
 {Let $f(x,y)$ be a real-valued function with domain $D$ in $\Real{2}$, and let $(a,b)$ be a point in $D$. Let
  \textbf{v} be a unit vector in $\Real{2}$. Then the
...
\statethm{thm:dirderiv}{
 ...
 a point in $D$, and let $\textbf{v} = \vectwo{v}$ be a unit vector in $\Real{2}$. Then
...
\statedefn{defn:grad}{\index{gradient}
 {For a real-valued function $f(x,y)$, the \textbf{gradient} of $f$, denoted by $\nabla f$, is the vector
  ...
  \end{equation}
  in $\Real{3}$. The symbol $\nabla$ is pronounced ``del''.\footnotemark}
...
level curve $f(x,y) = c$ (see Figure \ref{fig:gradlevel}).\vspace{-1mm}\index{continuously differentiable}
\begin{figure}[h]
 \begin{center}
  \begin{tikzpicture}
   \usetikzlibrary{arrows}
...
In general, for any unit vector \textbf{v} in $\Real{2}$, we still have 
...
\statedefn{defn:localext}{\index{local maximum}\index{local minimum}\index{global maximum}\index{global minimum}
 {Let $f(x,y)$ be a real-valued function, and let $(a,b)$ be a point in the domain of $f$. We say that $f$ has a
  \textbf{local maximum} at $(a,b)$ if $f(x,y) \le f(a,b)$ for all $(x,y)$ inside some disk of positive radius centered
  at $(a,b)$, i.e. there is some sufficiently small $r > 0$ such that $f(x,y) \le f(a,b)$ for all $(x,y)$ for
  which $(x-a)^2 + (y-b)^2 < r^2$.
  
  Likewise, we say that $f$ has a \textbf{local minimum} at $(a,b)$ if $f(x,y) \ge f(a,b)$ for all $(x,y)$ inside some
  disk of positive radius centered at $(a,b)$.
  
  If $f(x,y) \le f(a,b)$ for all $(x,y)$ in the domain of $f$, then $f$ has a \textbf{global maximum} at $(a,b)$. If
  $f(x,y) \ge f(a,b)$ for all $(x,y)$ in the domain of $f$, then $f$ has a \textbf{global minimum} at $(a,b)$.}
}
...
\statethm{thm:extnec}{
 {Let $f(x,y)$ be a real-valued function such that both $\frac{\partial f}{\partial x}(a,b)$ and
 $\frac{\partial f}{\partial y}(a,b)$ exist. Then a necessary condition for $f(x,y)$ to have a local maximum or minimum
 at $(a,b)$ is that $\nabla f (a,b) = \textbf{0}$.}
}
...
The following theorem gives sufficient conditions for a critical point to be a local maximum or\index{$D$}
minimum of a \emph{smooth function}\index{smooth function} (i.e. a function whose partial derivatives of all orders
exist and are continuous), which we will not prove here.\footnote{See \cite[\S\,7.6]{tm}.}\index{Second Derivative Test}

\statethm{thm:extsuff}{
 {Let $f(x,y)$ be a smooth real-valued function, with a critical point at $(a,b)$ (i.e.
 $\nabla f(a,b) = \textbf{0}$). Define
 \begin{displaymath}
  D = \dfrac{\partial^2 f}{\partial x^2}(a,b) \, \dfrac{\partial^2 f}{\partial y^2}(a,b) -
 \biggl( \dfrac{\partial^2 f}{\partial y \, \partial x}(a,b) \biggr)^2
 \end{displaymath}
 Then
 \begin{enumerate}[(a)]
  \item if $D > 0$ and $\tfrac{\partial^2 f}{\partial x^2}(a,b) > 0$, then $f$ has a local minimum at $(a,b)$
  \item if $D > 0$ and $\tfrac{\partial^2 f}{\partial x^2}(a,b) < 0$, then $f$ has a local maximum at $(a,b)$
  \item if $D < 0$, then $f$ has neither a local minimum nor a local maximum at $(a,b)$
  \item if $D = 0$, then the test fails.
 \end{enumerate}}
}

If condition (c) holds, then $(a,b)$ is a \emph{saddle point}.\index{saddle point}
...
\hrule width \textwidth height 0.5pt
\begin{exmp}
 Find all local maxima and minima of $f(x,y) = x^2 + xy + y^2 - 3x$.\vspace{1mm}
...
For example, if one of the equations that had to be solved was\index{Newton's algorithm}
\begin{displaymath}
 x^3 + 9x - 2 = 0 ~,
\end{displaymath}
...
In the case of functions which have a global maximum or minimum, Newton's algorithm can be used to find those
...
of nonlinear programming in general, see \cite{bss}.\index{steepest descent}
...
\begin{exmp}\label{exmp:consimple}
 ...
 which we then substitute into $f$ to get $f(x,y) = xy = x(10-x) = 10x - x^2$. This is now a function of $x$ alone, so
 ...
 $f = 0$ at the endpoints of the interval). So since $y=10-x =5$, then the maximum area occurs for a rectangle whose
