...
If the set described by equation $g(x,y)=c$ is not bounded,
but the set $f(x,y)\le d$ is bounded for any $d$ 
...
the constraint $xy=1$.  
It has two extremal points $(1,1)$ with the Lagrange multiplier $1$ and the point $(-1,-1)$ with the Lagrange multiplier $-1$.
Non of these points is global maximum, nor minimum.
...
then only one point $(1,1)$ satisfies  is the constraint global minimum point.
...
\probs{C}
\resume{enumerate}[{[\bfseries 1.]}]
\item Let $(x_0,y_0)$ is a minimum point of smooth function $f(x,y)$ with the constraint $g(x,y)\le c$.
Assume $\nabla g(x,y)\ne \textbf{0}$ if $g(x,y)= c$.
Show that  
$\nabla f(x_0,y_0)=\lambda \nabla g(x_0,y_0)$
for some $\lambda$. 
Moreover, if $g(x_0,y_0)<c$ then $\lambda=0$.
