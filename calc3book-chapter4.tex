...
in $\Real{1}$. This integral (usually called a \emph{Riemann integral})\index{Riemann integral}
...
\statedefn{defn:lineintvec2}{
 For a vector field $\textbf{f}(x,y) = P(x,y)\,\textbf{i} + Q(x,y)\,\textbf{j}$ and a curve $C$ with a smooth
 parametrization $x=x(t)$, $y=y(t)$, $a \le t \le b$, the \textbf{line integral of f along} $C$ is
 \begin{align}
  \int_C \Dotprod{\textbf{f}}{d\textbf{r}} ~&=~ \int_C P(x,y)\,dx ~+~ \int_C Q(x,y)\,dy\\
   &=~ \int_a^b \Dotprod{\textbf{f}(x(t),y(t))}{\textbf{r}\,'(t)}\,dt ~,
 \end{align}
 where $\textbf{r}(t) = x(t)\,\textbf{i} + y(t)\,\textbf{j}$ is the position vector for points on $C$.
}
We use the notation $d\textbf{r} = \textbf{r}\,'(t)\,dt = dx\,\textbf{i} + dy\,\textbf{j}$\index{$d\textbf{r}$} to
...
\statethm{thm:lineinttangent}{
 For a vector field $\textbf{f}(x,y) = P(x,y)\,\textbf{i} + Q(x,y)\,\textbf{j}$ and a curve $C$ with a smooth
 parametrization $x=x(t)$, $y=y(t)$, $a \le t \le b$ and position vector $\textbf{r}(t) = x(t)\,\textbf{i} +
 y(t)\,\textbf{j}$,
 \begin{equation}\label{eqn:lineintform2}
  \int_C \Dotprod{\textbf{f}}{d\textbf{r}} ~=~ \int_C \Dotprod{\textbf{f}}{\textbf{T}}\,ds ~,
 \end{equation}
 where $\textbf{T}(t) = \frac{\textbf{r}\,'(t)}{\norm{\textbf{r}\,'(t)}}$ is the unit tangent vector to $C$ at
 $(x(t),y(t))$.
}
If the vector field $\textbf{f}(x,y)$ represents the force moving an object along a curve $C$, then the work
$W$ done by this force is
\begin{equation}\label{eqn:work}
 W ~=~ \int_C \Dotprod{\textbf{f}}{\textbf{T}}\,ds ~=~ \int_C \Dotprod{\textbf{f}}{d\textbf{r}} ~.
\end{equation}
...
\statethm{thm:lineintreparam}{
 ...
 increasing on $\ival{c}{d}$). Then $\int_C \Dotprod{\textbf{f}}{d\textbf{r}}$ has the same value for the
...
multivariable functions:\index{Chain Rule}

\statethm{thm:chainrule2}{
 {(\textbf{Chain Rule})
 If $z=f(x,y)$ is a continuously differentiable function of $x$ and $y$, and both $x=x(t)$ and $y=y(t)$ are
...
\statethm{thm:lineintsuffpath}{
 {Let $\textbf{f}(x,y) = P(x,y)\,\textbf{i} + Q(x,y)\,\textbf{j}$ be a vector field in some region $R$, with $P$
...
for \textbf{f}. A \textbf{conservative} vector field is one which has a potential.\index{potential}
...
\statecor{cor:lineintsuffpath}{
 {If a vector field $\textbf{f}$ has a potential in a region $R$, then $\displaystyle\olineintvec{C}{f}{r} = 0$
 for any closed curve $C$ in $R$ (i.e. $\displaystyle\oint_{C}\Dotprod{\nabla F}{d\textbf{r}} = 0$ for any real-valued
 function $F(x,y)$).}
}
...
\statethm{thm:green}{
 {(\textbf{Green's Theorem})
  ...
  Let $\textbf{f}(x,y) = P(x,y)\,\textbf{i} + Q(x,y)\,\textbf{j}$ be a smooth vector field defined on both
  $R$ and $C$. Then
...
side.\index{multiply connected}\vspace{-1mm}
...
shown:\index{path independence}\vspace{3mm}
\statecomment{
 {The following statements are equivalent for a simply connected region $R$ in $\Real{2}$:
  \begin{enumerate}[(a)]
  \item $\textbf{f}(x,y) ~=~ P(x,y)\,\textbf{i} + Q(x,y)\,\textbf{j}$ has a smooth potential $F(x,y)$ in $R$
...
\newpage
%\startexercises
\centerline{\fbox{\textsf{\textbf{\large Exercises}}}}\label{sec4dot3}
\probs{A}
...
Since $\textbf{r}(u,v)$ is a function of two variables, define the partial derivatives
...
\begin{equation}
 ...
 \iint\limits_{R}\,\NORM{\Crossprod{\frac{\partial \textbf{r}}{\partial u}}{\frac{\partial \textbf{r}}{\partial v}}}
 \,du\,dv ~.
\end{equation}
...
\statedefn{defn:surfintreal}{
 {Let $\Sigma$ be a surface in $\Real{3}$ parametrized by $x=x(u,v)$, $y=y(u,v)$,\\$z=z(u,v)$, for $(u,v)$ in some
  region $R$ in $\Real{2}$. Let $\textbf{r}(u,v) = x(u,v) \textbf{i} + y(u,v) \textbf{j} + z(u,v) \textbf{k}$ be the
  ...
  \begin{equation}\label{eqn:surfintreal}
   ...
   \NORM{\Crossprod{\frac{\partial \textbf{r}}{\partial u}}{\frac{\partial \textbf{r}}{\partial v}}}\,du\,dv ~.
  \end{equation}
...
Since $\frac{\partial \textbf{r}}{\partial u}$ and $\frac{\partial \textbf{r}}{\partial v}$ are tangent to the surface
...
We say that \textbf{n} is a \textbf{normal vector} to $\Sigma$.\index{vector!normal}
...
\statedefn{defn:surfintvec}{
 {Let $\Sigma$ be a surface in $\Real{3}$ and let $\textbf{f}(x,y,z) = \ssub{f}{1}(x,y,z) \textbf{i} +
  \ssub{f}{2}(x,y,z) \textbf{j} + \ssub{f}{3}(x,y,z) \textbf{k}$ be a vector field defined on some subset
  of $\Real{3}$ that contains $\Sigma$. The \textbf{surface integral} of \textbf{f} over $\Sigma$ is
  \begin{equation}\label{eqn:surfintvec}
   \iint\limits_{\Sigma} \Dotprod{\textbf{f}}{d\bm{\sigma}} ~=~
   \iint\limits_{\Sigma} \Dotprod{\textbf{f}}{\textbf{n}}\,d\sigma ~,
  \end{equation}
...
outward unit normal vector at each point of $\Sigma$ changes. The following theorem provides an easier way
in the case
...
\statethm{thm:divergence}{
 {(\textbf{Divergence Theorem})\,\index{Divergence Theorem}
  Let $\Sigma$ be a closed surface in $\Real{3}$ which bounds a solid $S$, and let $\textbf{f}(x,y,z) =
 \ssub{f}{1}(x,y,z) \textbf{i} +
  \ssub{f}{2}(x,y,z) \textbf{j} + \ssub{f}{3}(x,y,z) \textbf{k}$ be a vector field defined on some subset
  of $\Real{3}$ that contains $\Sigma$. Then
  \begin{equation}\label{eqn:divergence}
   \iint\limits_{\Sigma} \Dotprod{\textbf{f}}{d\bm{\sigma}} ~=~ \iiint\limits_{S} \text{div}~\textbf{f} ~dV ~,
  \end{equation}
  where
  \begin{equation}\label{eqn:div}
   \text{div}~\textbf{f} ~=~ \frac{\partial \ssub{f}{1}}{\partial x} + \frac{\partial \ssub{f}{2}}{\partial y} +
...
\hrule width \textwidth height 0.5pt
\begin{exmp}
 Evaluate $\iint\limits_{\Sigma} \Dotprod{\textbf{f}}{d\bm{\sigma}}$, where $\textbf{f}(x,y,z) = x\textbf{i} +
  y\textbf{j} + z\textbf{k}$ and $\Sigma$ is the unit sphere $x^2 + y^2 + z^2 = 1$.\vspace{1mm}
 \par\noindent \emph{Solution:} We see that $\text{div}~\textbf{f} = 1+1+1=3$, so
 \begin{align*}
  \iint\limits_{\Sigma} \Dotprod{\textbf{f}}{d\bm{\sigma}} ~&=~
  \iiint\limits_{S} \text{div}~\textbf{f} ~dV
...
In physical applications, the surface integral $\iint\limits_{\Sigma} \Dotprod{\textbf{f}}{d\bm{\sigma}}$ is often
...
\statethm{thm:divzero}{
 {If the flux of a vector field \textbf{f} is zero through every closed surface containing a given point,
  then $\text{div}~\textbf{f} = 0$ at that point.}
}
\begin{proofbar}\vspace{-3mm}\begin{proof}[Proof:]
...
\statedefn{defn:lineintvec3}{
 For a vector field $\textbf{f}(x,y,z) = P(x,y,z)\,\textbf{i} + Q(x,y,z)\,\textbf{j} + R(x,y,z)\,\textbf{k}$ and
 a curve $C$ in $\Real{3}$ with a smooth parametrization $x=x(t)$, $y=y(t)$, $z=z(t)$, $a \le t \le b$, the \textbf{line
 integral of f along} $C$ is
 \begin{align}
  \int_C \Dotprod{\textbf{f}}{d\textbf{r}} ~&=~ \int_C P(x,y,z)\,dx ~+~ \int_C Q(x,y,z)\,dy ~+~ \int_C R(x,y,z)\,dz\\
   &=~ \int_a^b \Dotprod{\textbf{f}(x(t),y(t),z(t))}{\textbf{r}\,'(t)}\,dt ~,
 \end{align}
 where $\textbf{r}(t)= x(t)\,\textbf{i} + y(t)\,\textbf{j} + z(t)\,\textbf{k}$ is the position vector for points on $C$.
}

Similar to the two-variable case, if $\textbf{f}(x,y,z)$ represents the force applied to an object at a point $(x,y,z)$
...
\statethm{thm:lineintsuffpath3}{
 {Let $\textbf{f}(x,y,z) = P(x,y,z)\,\textbf{i} + Q(x,y,z)\,\textbf{j} + R(x,y,z)\,\textbf{k}$ be a vector field
...
\statecor{cor:lineintsuffpath3}{
 {If a vector field $\textbf{f}$ has a potential in a solid $S$, then $\displaystyle\olineintvec{C}{f}{r} = 0$
...
\begin{exmp}\label{exmp:conhelix}
 ...
 $C$ is called a \emph{conical helix}\index{conical helix}\index{helix}. See Figure \ref{fig:conhelix}).\vspace{1.5mm}
...
that such an \textbf{N} is a \emph{normal vector field}.\index{normal vector field}\index{vector
field!normal}
...
\piccaption[]{}\parpic[r]{\begin{tikzpicture}
 ...
 \fill (0.98,0.98) circle (2pt);
 \draw [line width=1.2pt,-latex] (0.98,0.98) -- (1.5,1.5);
 \node [right] at (1.5,1.5) {\small $\textbf{N}$};
 \draw [dashed,line width=1.2pt,-latex] (0.98,0.98) -- (0.46,0.46);
 \node [above] at (0.4,0.58) {\small $-\textbf{N}$};
\end{tikzpicture}}
...
(see Figure \ref{fig:mobius}).\vspace{-2mm}
...
For an orientable surface $\Sigma$ which has a boundary curve $C$, pick a unit normal vector \textbf{n}
such that if you walked along $C$ with your head pointing in the direction of \textbf{n}, then the surface would be on
your left. We say in this situation that \textbf{n} is a \emph{positive unit normal vector}\index{vector!positive unit
normal} and that $C$ is traversed \emph{\textbf{n}-positively}\index{n-positive@\textbf{n}-positive direction}. We can
...
\statethm{thm:stokes}{
 {(\textbf{Stokes' Theorem})\,\index{Stokes' Theorem}
  ...
  $\textbf{f}(x,y,z) = P(x,y,z) \textbf{i} + Q(x,y,z) \textbf{j} + R(x,y,z) \textbf{k}$ be a smooth vector field
  ...
  \begin{equation}\label{eqn:curl}
   \text{curl}~\textbf{f} ~=~ \left( \frac{\partial R}{\partial y} - \frac{\partial Q}{\partial z} \right) \textbf{i} ~+~
    \left( \frac{\partial P}{\partial z} - \frac{\partial R}{\partial x} \right) \textbf{j} ~+~
    \left( \frac{\partial Q}{\partial x} - \frac{\partial P}{\partial y} \right) \textbf{k} ~,\index{curl}
  \end{equation}
  \textbf{n} is a positive unit normal vector over $\Sigma$, and $C$ is traversed \textbf{n}-positively.}
}
\begin{proofbar}\vspace{-3mm}\begin{proof}[Proof:]
 ...
 $D$ in $\Real{2}$. \piccaption[]{}\parpic[r]{\begin{tikzpicture}
  ...
  \node [right] at (2.7,2.3) {\small $\textbf{n}$};
...
Note: The condition in Stokes' Theorem that the surface $\Sigma$ have a (continuously varying) positive unit normal
...
\begin{exmp}\label{exmp:stokesparab}
 ...
 \piccaption[]{\quad $z=x^2 + y^2$}\parpic[r]{\begin{tikzpicture}
  ...
  \pgfputat{\pgfpointxyz{0.2}{0}{1.8}}{\pgfbox[center,center]{\small $x$}};
  \pgfputat{\pgfpointxyz{0.05}{-0.2}{0}}{\pgfbox[center,center]{\small $0$}};
  \node [right] at (0.7,3.15) {\small $\textbf{n}$};
...
is often called the \textbf{circulation}\index{circulation} of \textbf{f} around $C$. For example, if \textbf{E}
...
\piccaption[]{\quad Curl and rotation}\parpic[r]{\begin{tikzpicture}[scale=1.34]
 ...
 \node [right] at (2.25,2.77) {$\textbf{f}$};
\end{tikzpicture}}
...
\statecomment{
 {The following statements are equivalent for a simply connected solid region $S$ in $\Real{3}$:
  \begin{enumerate}[(a)]
  \item $\textbf{f}(x,y,z) = P(x,y,z)\,\textbf{i} + Q(x,y,z)\,\textbf{j} + R(x,y,z)\,\textbf{k}$ has a smooth potential
   $F(x,y,z)$ in $S$
  ...
  \item $\dfrac{\partial R}{\partial y} = \dfrac{\partial Q}{\partial z}$,
   ...
   $\dfrac{\partial Q}{\partial x} = \dfrac{\partial P}{\partial y}$ in $S$ (i.e. $\text{curl}~\textbf{f} =
   \textbf{0}$ in $S$)
...
\resume{enumerate}[{[\bfseries 1.]}]
 \begin{multicols}{2}
 \item $\textbf{f}(x,y,z) = y\,\textbf{i} - x\,\textbf{j} + z\,\textbf{k}$
 \item $\textbf{f}(x,y,z) = a\,\textbf{i} + b\,\textbf{j} + c\,\textbf{k}$ ($a$, $b$, $c$ constant)
 \end{multicols}
 \begin{multicols}{2}
 \item $\textbf{f}(x,y,z) = (x+y)\,\textbf{i} + x\,\textbf{j} + z^2 \,\textbf{k}$
 \item $\textbf{f}(x,y,z) = xy\,\textbf{i} - (x-yz^2 )\,\textbf{j} + y^2 z\,\textbf{k}$
 \end{multicols}
\suspend{enumerate}
\probs{B}
...
\statecor{cor:curlgrad0}{
 {If a vector field $\textbf{f}(x,y,z)$ has a potential, then $\text{curl}~\textbf{f} = \textbf{0}$.
 }
}
...
\statethm{thm:divcurl0}{
 {For any smooth vector field $\textbf{f}(x,y,z)$, $\Dotprod{\nabla}{(\Crossprod{\nabla}{\textbf{f}})} = 0$.
 }
}
The proof is straightforward and left as an exercise for the reader.
\newpage
\statecor{cor:divcurl0}{
 {The flux of the curl of a smooth vector field $\textbf{f}(x,y,z)$ through any closed surface is zero.
 }
}
\begin{proofbar}\vspace{-3mm}\begin{proof}[Proof:]
...
$\textbf{e}_{\theta}$, $\textbf{e}_{\phi}$ are orthonormal.\index{$\textbf{e}_{r}$, $\textbf{e}_{\theta}$,
$\textbf{e}_{z}$, $\textbf{e}_{\rho}$, $\textbf{e}_{\phi}$} By the right-hand rule, we see that
...
\newpage
\statecomment{
 {\textbf{Cartesian} $(x,y,z)$: Scalar function $F$; Vector field $\textbf{f} = \ssub{f}{1}\,\textbf{i} +
 \ssub{f}{2}\,\textbf{j} + \ssub{f}{3}\,\textbf{k}$
 \begin{alignat*}{2}
  \text{gradient} &: & \nabla F ~&=~ \frac{\partial F}{\partial x}\,\textbf{i} ~+~ \frac{\partial F}{\partial y}\,\textbf{j}
   ~+~ \frac{\partial F}{\partial z}\,\textbf{k}\\
  \text{divergence} &: & \Dotprod{\nabla}{\textbf{f}} ~&=~ \frac{\partial \ssub{f}{1}}{\partial x} ~+~
...
\vspace{3mm}
\statecomment{
 {\textbf{Cylindrical} $(r,\theta ,z)$: Scalar function $F$; Vector field $\textbf{f} =
 f_{r}\,\textbf{e}_{r} + f_{\theta}\,\textbf{e}_{\theta} + f_{z}\,\textbf{e}_{z}$
 \begin{alignat*}{2}
...
\vspace{3mm}
\statecomment{
 {\textbf{Spherical} $(\rho,\theta ,\phi)$: Scalar function $F$; Vector field $\textbf{f} =
 f_{\rho}\,\textbf{e}_{\rho} + f_{\theta}\,\textbf{e}_{\theta} + f_{\phi}\,\textbf{e}_{\phi}$
 \begin{alignat*}{2}
...
Now, since the angle $\theta$ is measured in the $xy$-plane, then the unit vector $\textbf{e}_{\theta}$ in the $\theta$
